...
export default function CoachDashboard() {
  ...
  const ourTeam = (teams[0] || { name: "T1", win: false, kills: 0, gold: 0, towers: 0, dragons: 0, barons: 0, heralds: 0, players: [] }) as any;
  const enemyTeam = (teams[1] || { name: "Enemy", win: false, kills: 0, gold: 0, towers: 0, dragons: 0, barons: 0, heralds: 0, players: [] }) as any;
  ...
  const teamAssists = (ourTeam.players as any[]).reduce((sum, p) => sum + (p.stats.assists || 0), 0);
  const teamKDA = teamDeaths > 0 ? ((teamKills + teamAssists) / teamDeaths).toFixed(1) : "Perfect";

  // Resource usage (based on gold efficiency)
  const totalGoldOurs = (ourTeam.players as any[]).reduce((sum, p) => sum + (p.stats.gold || 0), 0);
